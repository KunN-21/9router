 ...
 *
 *     let parsed = {};
 *     try { parsed = parseTOML(await fs.readFile(configPath, "utf-8")); }
 ...
 * the user had; for the Copilot provider array it drops every other model provider
 * the user had configured. Only ENOENT means "start fresh".
 *
 * Local note: codex-settings sends the API key as a static `http_headers` entry and
 * keeps the subagent model as scalar `agents.default_subagent_model` — there is no
 * `auth.json` write path to guard here, only `config.toml`.
 */
...
describe("codex-settings uses it on the config write path", () => {
  it("no longer treats an unreadable config file as empty, keeps local static-key shape", () => {
    const src = fs.readFileSync(
    ...
    expect(src).not.toContain("catch { /* No existing config */ }");
    // Local behavior preserved: custom providers ignore auth.json, the key travels
    // as a static header and the subagent model stays a scalar.
    expect(src).toContain("http_headers");
    expect(src).toContain("agents.default_subagent_model");
    expect(src).not.toContain("catch { /* No existing auth */ }");
    // DELETE stays surgical: drops only 9Router keys, keeps other auth entries.
    expect(src).toContain("delete authData.OPENAI_API_KEY");
    // The refusal surfaces instead of a generic 500.
    expect(src).toContain("refusing to overwrite it");
  });
});
