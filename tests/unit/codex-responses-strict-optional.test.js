...
describe("codex responses strict optional tools", () => {
  ...
  it("normalizes Chat tool_choice to Responses-native named function", () => {
    const body = chatBody();
    body.tool_choice = { type: "function", name: "EnterWorktree" };
    // Local master normalizes (d0a74dd8e) instead of dropping: the PR parent
    // had no normalizeToolChoice, so this expectation was toBeUndefined there.
    expect(openaiToOpenAIResponsesRequest("m", body, true, {}).tool_choice).toEqual({ type: "function", name: "EnterWorktree" });
  });
